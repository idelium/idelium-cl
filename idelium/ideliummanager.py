...
from idelium.thirdparties.ideliumpostman import PostmanCollection
...
class StartManager:
    ...
    def execute_step(driver, config):
        ...
        for object_step in config["json_step"]["steps"]:
            if status == "1":
                if object_step['stepType'] == 'bdd':
                    print ('is Bdd')
                    print (object_step['bddConfig'])
                    sys.exit(1) 
                elif object_step['stepType'] == 'postman_collection':
